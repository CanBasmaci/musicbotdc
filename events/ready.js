...
module.exports = async (client) => {
  ...
  if (config.mongodbURL || process.env.MONGO) {
    ...
    setInterval(() => client.user.setActivity({ name: `${config.status} - Bizim için ${Number(client?.shard?.ids) + 1 ? Number(client?.shard?.ids) + 1 : "Aktif"}`, type: ActivityType.Listening }), 10000);
